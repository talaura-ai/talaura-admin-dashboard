  ...
  if (isSuccess) {
    return (
      <div className="w-full">
        {candidatesData?.Candidate.length === 0 ? (
          <div className="w-full min-h-[500px] flex justify-center items-center">
            <h2 className="text-3xl">No Candidates found</h2>
          </div>
        ) : (
          <Fragment>
            <Header
                              ...
                              <Fragment>
                                ...
                                {(() => {
                                  if (allTypesInResponseSet.has('quiz')) {
                                    ...
                                    return (
                                      <td
                                        className={`whitespace-nowrap px-3 py-4 text-sm text-center ${statusModule === 'Interrupted' ? 'text-[#FB2121]' : statusModule === 'Completed' || statusModule === 'Ongoing' ? 'text-[#40B24B]' : 'text-[#7D7C7C]'}`}
                                      >
                                        {statusModule}
                                      </td>
                                    );
                              ...
                              <Fragment>
                                ...
                                {(() => {
                                  if (allTypesInResponseSet.has('text to text')) {
                                    const statusModule = candidate?.module?.find(
                                      (mdl) =>
                                        mdl.type.toLowerCase().toLowerCase() === 'voice to text',
                                    )?.status;
                                    return (
                                      <td
                                        className={`whitespace-nowrap px-3 py-4 text-sm text-center ${statusModule === 'Interrupted' ? 'text-[#FB2121]' : statusModule === 'Completed' || statusModule === 'Ongoing' ? 'text-[#40B24B]' : 'text-[#7D7C7C]'}`}
                                ...
                                {(() => {
                                  if (allTypesInResponseSet.has('ai video interview')) {
                                    const statusModule = candidate?.module?.find(
                                      (mdl) =>
                                        mdl.type.toLowerCase().toLowerCase() ===
                                        'ai video interview',
                                    )?.status;
                                    return (
                                      <td
